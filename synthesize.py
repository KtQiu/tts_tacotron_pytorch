...
from util import inv_spectrogram, find_endpoint, save_wav, spectrogram
import numpy as np
import argparse
import os
import sys
...
def main(args):
    ...
    for i, text in enumerate(sentences):
        ...
        path = os.path.join(tacotron_hparams["output_path"],
                            'result_%d_%d.wav' % (args.restore_step, i + 1))
...
def generate(model, text):
    ...
    # Variables
    characters = Variable(
        torch.from_numpy(seq).type(torch.cuda.LongTensor),
        volatile=True).cuda()
    mel_input = Variable(
        torch.from_numpy(mel_input).type(torch.cuda.FloatTensor),
        volatile=True).cuda()
